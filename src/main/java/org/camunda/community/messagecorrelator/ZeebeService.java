...
public class ZeebeService {
  ...
  public ZeebeService(ZeebeClient zeebeClient) throws ConfigurationException {
    ...
    this.syncMessage = config.getString("mc.syncMessage");
    this.jobTypePrefix = config.getString("mc.syncTaskTypePrefix");
    String messagesProcessVarFromConfig = config.getString("mc.messagesProcessVar");
    this.messagesProcessVar = Objects.requireNonNullElse(messagesProcessVarFromConfig, "messages");

    String bpmnPath = config.getString("mc.path");
  ...
  private Mono<Map<String, Object>> getProcessVariables(String parcelId) {
    Mono<Map<String, Object>> processVariables =
        Mono.create(
            sink -> {
              String jobType = jobTypePrefix + parcelId;
              JobWorker worker =
                  zeebeClient
                      .newWorker()
                      .jobType(jobType)
